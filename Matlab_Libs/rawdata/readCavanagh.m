function [EEG, Seq, seqidx, All_data] = readCavanagh(SubjectNum, filter)
    %{
    Inputs:
    Subject Number
    Task Number
    ...
    if ispc
        ...
        BasePath = fullfile(getenv('HOME'), 'DS', 'ds003490-download', sprintf('sub-%03d', SubjectNum));
    end
    ...
    for runnumber = 1:2
        ...
        EEG_data = pop_loadset(fullfile(BasePath, sprintf('ses-%02d', runnumber), 'eeg', ...
            sprintf('sub-%03d_ses-%02d_task-Rest_eeg.set', SubjectNum, runnumber)));
        temp = EEG_data.data;
        EEG{runnumber} = temp;
