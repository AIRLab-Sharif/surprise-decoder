function out = IdealObserver(in)
    ...
    %       'WithFirst', the 1st sample is assumed to depend on tion
    ...
    indefaults.opt.ReturnPuzzlement = 0; % compute gerstner surprise
    ...
    elseif strcmpi(in.learned, 'transition') && in.jump == 0
        ...
        in = CheckInput(in, indefaults, 'ReturnDistUpdate');
        in = CheckInput(in, indefaults, 'ReturnPuzzlement');
        ...
        if in.opt.ReturnDistUpdate
            ...
            out.distUpdate2 = UpdateBetaJoint2(...
                cat(2, in.opt.priorp1g2', [p1g2_a; p1g2_b]), ...
                cat(2, in.opt.priorp2g1', [p2g1_a; p2g1_b]));
        else
            out.distUpdate = [];
        ...
        % get puzzlement (confidence corrected) surprise (gerstner)
        if in.opt.ReturnPuzzlement == 1
            %calculate p_hat
            [ph1g2_a, ph1g2_b, ph2g1_a, ph2g1_b] = calculate_p_hat(in);

            out.puzzlement = KLBetaListJoint(...
                cat(2, in.opt.priorp1g2', [p1g2_a(1:end - 1); p1g2_b(1:end - 1)]), ...
                cat(2, in.opt.priorp2g1', [p2g1_a(1:end - 1); p2g1_b(1:end - 1)]), ...
                [ph1g2_a; ph1g2_b], ...
                [ph2g1_a; ph2g1_b]);

            out.puzzlement2 = KLBetaListJoint(...
                [p1g2_a; p1g2_b], [p2g1_a; p2g1_b], ...
                [ph1g2_a; ph1g2_b], [ph2g1_a; ph2g1_b]);
        else
            out.puzzlement = [];
        end
    ...
    elseif strcmpi(in.learned, 'frequency') && in.jump == 0
        ...
        if in.opt.ReturnDistUpdate
            ...
            out.distUpdate2 = UpdateBeta2(cat(2, in.opt.priorp1', [p1_a; p1_b]));
        else
            out.distUpdate = [];
        end
        ...
        if in.opt.ReturnPuzzlement == 1
            %calculate p_hat
            [ph1_a, ph1_b] = calculate_p_hat_bernouli(in);

            out.puzzlement = KLBetaList(...
                cat(2, in.opt.priorp1', [p1_a(1:end - 1); p1_b(1:end - 1)]), ...
                [ph1_a; ph1_b]);

            out.puzzlement2 = KLBetaList(...
                [p1_a; p1_b], ...
                [ph1_a; ph1_b]);
...
function kl = KLBeta(p, q)
    ...
    try
        kl = gammaln(sum(p)) - gammaln(sum(q)) ...
            -sum(gammaln(p)) + sum(gammaln(q)) ...
            +(p - q) * (psi(p) - psi(sum(p)))';
    catch
...
% Analytical KL divergence for Beta distributions
function KLdiv = UpdateBeta2(all_param)
    % Initialize variables
    seqL = size(all_param, 2);
    KLdiv = nan(1, seqL);
...
% Analytical KL divergence for Beta distributions
function KLdiv = KLBetaList(P, Q)

    if size(P, 2) ~= size(Q, 2)
        fprintf('error on P and Q size')
    end
...
% Analytical KL divergence for joint Beta distributions
function KLdiv = UpdateBetaJoint2(all_param_d1, all_param_d2)
    % Note: KL is additive for independent distributions.
    KLdiv1 = UpdateBeta2(all_param_d1);
    KLdiv2 = UpdateBeta2(all_param_d2);
    KLdiv = KLdiv1 + KLdiv2;
end

% Analytical KL divergence for joint Beta distributions
function KLdiv = KLBetaListJoint(P1, P2, Q1, Q2)
    % Note: KL is additive for independent distributions.
    KLdiv1 = KLBetaList(P1, Q1);
    KLdiv2 = KLBetaList(P2, Q2);
    KLdiv = KLdiv1 + KLdiv2;
end
...
function in = CheckInput(in, indefaults, action)
    ...
    switch action
        ...
        case 'ReturnPuzzlement'

            if ~isfield(in.opt, 'ReturnPuzzlement')
                in.opt.ReturnPuzzlement = indefaults.opt.ReturnPuzzlement;
                if in.verbose; fprintf(' \n use default for ''ReturnPuzzlement'''); end
            end
...
function [ph1g2_a, ph1g2_b, ph2g1_a, ph2g1_b] = calculate_p_hat(in)
    ph1g2_a = zeros(1, numel(in.s));
    ph1g2_b = zeros(1, numel(in.s));
    ph2g1_a = zeros(1, numel(in.s));
    ph2g1_b = zeros(1, numel(in.s));
    ...
    for k = 2:numel(in.s)
        last_2_seq = in.s(k - 1:k);
        [~, Marginal_mean, Marginal_sd] = MarkovNoJump(last_2_seq, [], ...
            in.opt.priorp1g2, in.opt.priorp2g1, in.opt.method, ...
            in.opt.MemParam, [], in.opt.AboutFirst);

        [p1g2_a, p1g2_b] = BetaMomentsToParams(Marginal_mean(1, :), Marginal_sd(1, :));
        [p2g1_a, p2g1_b] = BetaMomentsToParams(Marginal_mean(2, :), Marginal_sd(2, :));
...
function [ph1_a, ph1_b] = calculate_p_hat_bernouli(in)
    ph1_a = zeros(1, numel(in.s));
    ph1_b = zeros(1, numel(in.s));

    for k = 1:numel(in.s)
        last_seq = in.s(k);
        [~, out.p1_mean, out.p1_sd] = BernoulliNoJump(last_seq, in.opt.priorp1, ...
            in.opt.method, in.opt.MemParam, []);
